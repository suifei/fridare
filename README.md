...
- **host-wheels / pip**：tools 文件名必须是 PEP 440 本地版本，带 **`+`**。17：`frida_tools-14.10.4+frida.17.17.0.fridare.kxmwp-py3-none-any.whl`。16：`frida_tools-13.7.1+frida.16.7.19.fridare.kxmwp-py3-none-any.whl`（旧包 `frida_tools-tools.frida.16.7.19…` pip 拒装）。写成 `14.10.4.frida.` 也会报 `Invalid wheel filename`（[#36](https://github.com/suifei/fridare/issues/36)）。16/17 的 tools **不能混用**。
