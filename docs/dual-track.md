...
官方默认是 **27042**。server **启动时就可以指定** IP 和端口（`-l 0.0.0.0:28888`），客户机连 **同一个端口**（`frida -H 127.0.0.1:28888`）。  
**默认端口改不改都无所谓**；要避开常见扫描，用启动参数换一个端口即可，不必先改二进制。

源码魔改 **支持** 改 `DEFAULT_CONTROL_PORT`（可选，不是必须）：

| 情况 | 行为 |
|------|------|
| GUI / 配置端口仍是 **27042** | **不改**默认值（不写 27042→27042 空操作） |
| 配置了其它 **5 位**端口（如 27142） | 只改 `DEFAULT_CONTROL_PORT = 27042` 以及 **frida-core** 内同长度 ASCII；**禁止**全树替换裸数字（brotli/capstone/xz 等是假阳性） |
| 位数不同（如 9999） | 只改 `DEFAULT_CONTROL_PORT` 赋值 |

Agent 的 find **必须是官方 27042**，不能把用户端口当成原文。

预编译包 **[kxmwp-17.17.0](./kxmwp-17.17.0.md) 没有改二进制默认端口**。用法：启动带 `-l`，客户机写明连接端口。
