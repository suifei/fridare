...
官方默认是 **27042**（`frida-core` 的 `DEFAULT_CONTROL_PORT`）。源码魔改会把它当作 **L2 必做项**，但有边界：

| 情况 | 行为 |
|------|------|
| GUI / 配置端口仍是 **27042** | **不改**端口（不写 27042→27042 空操作） |
...
预编译包 **[kxmwp-17.17.0](./kxmwp-17.17.0.md) 没有改端口**，请按 27042 启动/连接。若要换端口：用静态「frida 魔改 / iOS DEB 魔改」再打一遍，或启动参数 `-l IP:新端口`，**连接时填你改过的端口**。
